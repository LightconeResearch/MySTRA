...
| Put a measured number in prose | [`{astra:value}`](live-values.md) | `` {astra:value col=alpha err=alpha_std}`outputs.table` `` |
...
The fit yields $\alpha =$ {astra:value col=alpha where="tracer=elg1" err=alpha_std}`outputs.alpha_table`
