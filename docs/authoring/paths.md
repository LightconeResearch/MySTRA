...
A **path** is a slash-separated route through the analysis tree — the same structure as `astra.yaml`. Read it like a file path; the first meaningful segment is a top-level `astra.yaml` collection. One grammar drives every surface: the `{astra}` role, the `{astra}` directive, the `{astra:*}` variants, and the [`#astra:` cross-reference scheme](cross-references.md).
